...
{
	double mu = 0.840, sigma = 0.396;
	double norm_factor = 2*(1+exp(-mu*mu/sigma))*sqrt(M_PI*sigma);
	return (exp(- pow(v-mu,2)/(2*sigma)) + exp(- pow(v+mu,2)/(2*sigma))) /norm_factor; //double gaussian wave function
	//return 1.0; //flat wave function
}
...
double variationalWaveFunction_second(double v)
{
	double mu = 0.840, sigma = 0.396;
	double norm_factor = 2*(1+exp(-mu*mu/sigma))*sqrt(M_PI*sigma);
	double kin_energy = -( pow(v-mu,2) * exp(-pow((v-mu),2)/(2*sigma) ) + pow(v+mu,2) * exp(-pow((v+mu),2)/(2*sigma)) )/ (2.*mass*sigma*sigma);
	kin_energy = kin_energy + ( exp(-pow((v-mu),2)/(2*sigma)) + exp(-pow((v+mu),2)/(2*sigma)) ) / (2.*sigma);
	kin_energy /= ( exp(- pow(v-mu,2)/(2*sigma)) + exp(- pow(v+mu,2)/(2*sigma)) );
	return kin_energy*2./norm_factor; //double gaussian wave function
	//return 0; //flat wave function
}
